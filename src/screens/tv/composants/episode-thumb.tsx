  ...
  return (
    <Thumb aspectRatio={16 / 15} imageUrl={imageUrl} imageWidth="w780" type="tv">
      <View className="absolute left-0 right-0 bottom-0 top-[30%] justify-end bg-linear-180 from-transparent via-background-fixed/80 to-background-fixed/95 p-3 gap-0.5">
        <Text className="uppercase" variant="sm">
          <FormattedMessage defaultMessage="Episode {number}" id="xf6y+C" values={{ number }} />
        </Text>
        <Text bold className="text-text-maximal" numberOfLines={overview ? 1 : 2} variant="lg">
          {name}
        </Text>
        {overview && (
          <Text className="leading-tight" numberOfLines={3} variant="sm">
            {overview}
          </Text>
        )}
        <View className="flex-row items-center">
          {runtime && runtime > 0 ? (
            <Text className="text-text-maximal" variant="sm">
              {formatTime(runtime)}
            </Text>
          ) : undefined}
          {airDate && (
